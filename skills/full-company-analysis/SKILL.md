...
description: >
  ORCHESTRATOR. Run the end-to-end equity research pipeline for one ticker:
  intake → SEC → statements → dilution (if converts) → DCF → comps → LBO (lite)
  → SOTP (if multi-segment) → competitive → unit economics → scenarios →
  catalysts → risk audit → memo → slides. Triggers: "check out AAPL", "analyze
  this company: NVDA", "full analysis of MSFT", "run the pipeline on TSLA",
  "equity research studio on X".
version: "1.1.0"
---
...
| User says | Action |
|-----------|--------|
| check out / analyze / research {TICKER} | Full pipeline |
| quick look at {TICKER} | Lite pipeline |
| just DCF for {TICKER} | Hand off to `dcf-model` only |
| slides for {TICKER} | Require memo exists or build lite memo first |
