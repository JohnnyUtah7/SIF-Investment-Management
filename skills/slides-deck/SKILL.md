...
name: slides-deck
description: >
  Produce a Google Slides-ready research deck from analysis artifacts: write
  slide.md (markdown source), then render via gws / Google Slides skill, or
  fall back to PPTX. Use when user wants slides, a pitch deck, or as the final
  stage of full-company-analysis.
version: "1.1.0"
---
...
- Prefer **markdown slide source first**, then render.
...
## Render path (prefer in order)
1. **Native Google Slides** — if `gws` authenticated: follow p-md-to-slides patterns; write URL to `deck_link.txt`
2. **google-slides-skill** / Slides API OAuth — same
3. **PPTX fallback** — generate PPTX under `05-deck/deck.pptx` (or symlink); user uploads to Drive → Open with Google Slides
4. Deliver `slide.md` alone if no credentials — still valuable; `05-deck/` must not be empty
...
- Google OAuth / `gws auth login` must be done before live demos.
